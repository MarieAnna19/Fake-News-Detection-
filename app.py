...
def predict_fake_news(title, content):
    ...
    prediction = 0.4 * rf_combined_model.predict_proba(combined_features)[0][1] + 0.4 * rf_model.predict_proba(tfidf_features)[0][1] + 0.2 * rf_fll_model.predict_proba(emp_features)[0][1]
    ...
    if st.session_state.user_id is None:
        ...
        with st.sidebar:
            ...
            with st.expander("ℹ️ How It Works"):
                st.markdown("""
                    This Fake News Detector analyzes article text using a multi-step process:
                    1.  **Input:** You provide the article's title and content.
                    2.  **Preprocessing:** The text is cleaned (URLs, extra spaces removed, converted to lowercase) and common words (stopwords) are removed. Words are also reduced to their root forms (lemmatization).
                    3.  **Feature Extraction:** Key characteristics are identified:
                        * **TF-IDF:** Measures how important a word is to an article.
                        * **First Letter Proportion:** Analyzes the distribution of the first letters of words.
                    4.  **AI Analysis:** These features are processed by an ensemble of machine learning models (Random Forests).
                    5.  **Prediction:** The models output a 'realness' score.
                """)

            with st.expander("📊 Understanding the Score"):
                st.markdown("""
                    The 'realness' score ranges from **0.0 (likely Fake) to 1.0 (likely Real)**.
                    * A score closer to **0.0** suggests the article has characteristics commonly found in unreliable news.
                    * A score closer to **1.0** suggests the article aligns with patterns typical of reliable news.

                    **General Guide (as shown in results):**
                    * **Score < 0.3:** ⚠️ Likely Unreliable
                    * **Score 0.3 - 0.6:** 🧐 Exercise Caution / Questionable
                    * **Score > 0.6:** ✅ Likely Reliable

                    **Disclaimer:** This tool is an aid and not a definitive judge of truth. The prediction is based on learned patterns and should be one of many tools you use to assess information. Always consider the source and cross-reference with other reputable outlets.
                """)
...
def check_article_page():
    ...
    if st.session_state.current_prediction is not None:
        ...
        with col2:
            ...
            # Add color legend bar above the gradient
            st.markdown(f"""
            <div style="text-align: center; margin-bottom: 10px;">
                <div style="display: flex; width: 100%; height: 20px; border-radius: 5px; overflow: hidden;">
                    <div style="width: 30%; background-color: #d63031; display: flex; justify-content: center; align-items: center;">
                        <span style="color: white; font-size: 0.8em; font-weight: 500;">Likely Fake</span>
                    </div>
                    <div style="width: 30%; background-color: #f39c12; display: flex; justify-content: center; align-items: center;">
                        <span style="color: white; font-size: 0.8em; font-weight: 500;">Questionable</span>
                    </div>
                    <div style="width: 40%; background-color: #27ae60; display: flex; justify-content: center; align-items: center;">
                        <span style="color: white; font-size: 0.8em; font-weight: 500;">Likely Real</span>
                    </div>
                </div>
                <div style="display: flex; justify-content: space-between; margin-top: 2px; font-size: 0.7em; color: #666;">
                    <span>0.0</span>
                    <span>0.3</span>
                    <span>0.6</span>
                    <span>1.0</span>
...
def history_page():
    ...
    for i, item in enumerate(history):
        with st.expander(f"📄 {item['title']} - Score: {item['prediction']:.2f} - {item['date']}"):
            ...
            # Add legend bar above the gradient
            st.markdown(f"""
            <div style="margin-bottom: 8px;">
                <div style="display: flex; width: 100%; height: 15px; border-radius: 3px; overflow: hidden;">
                    <div style="width: 30%; background-color: #d63031;"></div>
                    <div style="width: 30%; background-color: #f39c12;"></div>
                    <div style="width: 40%; background-color: #27ae60;"></div>
                </div>
